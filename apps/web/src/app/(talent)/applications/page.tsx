...
export default function ApplicationsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <div className="text-xs uppercase tracking-[0.18em] text-slate-500 mb-2">Applications</div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Link href={`/jobs/${a.job.id}`} className="font-medium hover:text-brand-700 truncate">
                      {a.job.title}
                    </Link>
                ...
                <div className="text-right shrink-0">
                  <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[a.status] ?? 'bg-slate-100'}`}>
                    {a.status.toLowerCase().replace('_', ' ')}
                  </span>
                  {!['WITHDRAWN', 'HIRED', 'REJECTED'].includes(a.status) && (
                    <button
                      onClick={() => withdraw.mutate(a.id)}
                      className="block ml-auto mt-2 text-xs text-slate-500 hover:text-red-700"
                    >
                      Withdraw
                    </button>
                  )}
