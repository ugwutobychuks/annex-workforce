...
export default function PayslipsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <div className="text-xs uppercase tracking-[0.18em] text-slate-500 mb-2">Payslips</div>
        <h1 className="font-display text-4xl">Your earnings.</h1>
        <p className="text-slate-600 mt-2">Monthly breakdown of gross, deductions, and net pay.</p>
      </header>
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="text-slate-500">No payslips yet.</p>
            ...
            {payslips.map((p: any) => (
              <button
                ...
                className={`card w-full text-left transition-all ${
                  selectedId === p.id ? 'border-brand-700 ring-1 ring-brand-700' : 'hover:border-slate-300'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-medium font-mono">{p.payrollRun?.period}</div>
                    <div className="text-xs text-slate-500 mt-0.5">{p.contract?.jobTitle}</div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-400" />
                </div>
                <div className="mt-2 font-mono text-sm text-brand-900">
...
function PayslipDetail({ payslip }: { payslip: any }) {
  return (
    <div className="card relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-brand-700 via-amber-400 to-accent-500" />

      <div className="flex items-baseline justify-between border-b border-slate-200 pb-4 mb-6">
        <div>
          <div className="text-xs uppercase tracking-wider text-slate-500">Payslip</div>
          <div className="font-display text-2xl">{payslip.payrollRun?.period}</div>
        </div>
        <div className="text-xs text-slate-500 font-mono">PSL-{payslip.id.slice(0, 8).toUpperCase()}</div>
      </div>

      <div className="space-y-1">
        <div className="text-xs uppercase tracking-wider text-slate-500 mb-2">Position</div>
      ...
      <table className="w-full mt-6 text-sm">
        <tbody>
          <tr>
            <td className="py-2.5 text-slate-700">Gross salary</td>
          ...
          <tr className="font-semibold">
            ...
            <td className="text-right pt-4 font-mono text-brand-900 text-lg">
