...
export default function EmployerJobDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <Link href="/employer/jobs" className="inline-flex items-center gap-1 text-sm text-slate-600 hover:text-brand-700">
        <ArrowLeft className="w-4 h-4" /> Back to jobs
      </Link>

      <header className="flex items-start justify-between gap-4">
        <div>
          <div className="text-xs uppercase tracking-[0.18em] text-slate-500 mb-2">Applicants</div>
          ...
          <div className="text-sm text-slate-600 mt-2 flex items-center gap-3 flex-wrap">
            ...
            <span>·</span>
            <span className={`chip ${job?.status === 'PUBLISHED' ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-100'}`}>
      ...
      <div className="overflow-x-auto -mx-8 px-8">
        <div className="grid grid-flow-col auto-cols-[280px] gap-3 pb-4">
          {PIPELINE.map((stage) => (
            <div key={stage.key} className="bg-slate-100/70 rounded-lg p-3">
              <div className="flex items-center justify-between mb-3 px-1">
                <span className="text-xs font-medium uppercase tracking-wider text-slate-600">
                  {stage.label}
                </span>
                <span className="text-xs text-slate-500 font-mono">{byStatus[stage.key].length}</span>
              </div>
...
function ApplicantCard({ app, onClick, onMove }: any) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-md p-3 hover:border-brand-300 transition-colors relative">
      <button onClick={onClick} className="block w-full text-left">
        <div className="flex items-start gap-2">
          <div className="w-8 h-8 rounded-full bg-brand-50 border border-brand-100 flex items-center justify-center font-display text-xs font-semibold text-brand-900 shrink-0">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {(c.skills ?? []).slice(0, 3).map((s: any) => (
            <span key={s.id} className="text-[10px] px-1.5 py-0.5 rounded bg-brand-50 text-brand-900">
              {s.skill.name}
            </span>
          ))}
        </div>
        <div className="text-[10px] text-slate-400 mt-2">Applied {relativeTime(app.appliedAt)}</div>
      </button>
      <button
      ...
      {showMenu && (
        <div
          className="absolute top-8 right-2 bg-white border border-slate-200 rounded-md shadow-lg z-10 py-1 min-w-[140px]"
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-full bg-brand-50 border border-brand-100 flex items-center justify-center font-display text-xl font-semibold text-brand-900">
              {u.firstName?.[0]}{u.lastName?.[0]}
            </div>
          ...
          {app.coverLetter && (
            <section>
              ...
              <div className="text-sm text-slate-700 whitespace-pre-line leading-relaxed bg-slate-50 rounded-md p-4">
                {app.coverLetter}
              </div>
            </section>
          )}
